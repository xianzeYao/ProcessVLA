...
echo "[libero] trials_per_task=${NUM_TRIALS_PER_TASK} save_video=${SAVE_VIDEO} video_views=${VIDEO_VIEWS} image_views=${IMAGE_VIEWS}"
...
for index in "${!SUITE_LIST[@]}"; do
  ...
  worker_cmd=("${LIBERO_PYTHON}" "${EVAL_SCRIPT}" --args.pretrained-path "${CKPT}"
    ...
    --args.video-views "${VIDEO_VIEWS}" --args.image-views "${IMAGE_VIEWS}"
