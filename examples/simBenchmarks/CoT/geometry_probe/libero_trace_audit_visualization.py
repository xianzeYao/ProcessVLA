...
from .episode_video import write_video_frames
...
def _finite_limit(values: Sequence[np.ndarray], *, floor: float = 1.0) -> tuple[float, float]:
    ...
    if not len(finite):
        return (0.0, floor)
    high = max(float(np.max(finite)) * 1.15, floor)
    return (0.0, high)
...
def prepare_dashboard_view_model(record: RolloutRecord, frame_index: int) -> dict[str, object]:
    ...
    action_step = min(frame_index, len(record.executed_actions) - 1)
    anchor_index = max(0, int(np.searchsorted(record.anchor_steps, action_step, side="right") - 1))
    ...
    relative_step = int(np.clip(action_step - anchor_step, 0, horizon))
    ...
    view: dict[str, object] = {
        ...
        "cursor": {"action_step": action_step, "anchor_step": anchor_step, "chunk_point": point_index},
        "axis_limits": {
...
def _draw_triangle(axis: plt.Axes, uvd: np.ndarray, valid: np.ndarray, *, linestyle: str, label: str) -> None:
    points = np.asarray(uvd, float)
    visible = np.asarray(valid, bool) & np.isfinite(points).all(axis=1)
    visible &= (points[:, 0] >= 0.0) & (points[:, 0] <= 1.0) & (points[:, 1] >= 0.0) & (points[:, 1] <= 1.0)
    if not visible.all():
        return
...
def _build_dashboard_figure(record: RolloutRecord, view: Mapping[str, object]) -> plt.Figure:
    ...
    agent.set_axis_off()
    predicted = np.asarray(view["predicted_uvd"])[int(cursor["chunk_point"])]
    realized = np.asarray(view["realized_uvd"])[int(cursor["chunk_point"])]
    target_valid = np.asarray(view["target_valid"])[int(cursor["chunk_point"])]
    predicted_valid = np.isfinite(predicted).all(axis=1) & (predicted[:, 2] > 0)
    _draw_triangle(agent, predicted, predicted_valid, linestyle="-", label="predicted")
    _draw_triangle(agent, realized, target_valid, linestyle="--", label="simulator-realized")
...
def render_rollout_video(record: RolloutRecord, path: str | Path, fps: float = 10, *, summary_seconds: float = 2.0, frame_indices: Sequence[int] | None = None) -> Path:
    """Write per-anchor dashboard frames followed by a two-second summary card."""
    ...
    indices = list(range(len(record.agent_rgb))) if frame_indices is None else [
        _validate_frame_index(record, index) for index in frame_indices
    ]
    if not indices:
        raise ValueError("frame_indices must not be empty")
    card_count = int(round(float(fps) * float(summary_seconds)))
    last_state = len(record.agent_rgb) - 1
    def frames() -> Sequence[np.ndarray]:
        return [*(render_dashboard_frame(record, index) for index in indices), *(_summary_card(record, last_state) for _ in range(card_count))]
    output = Path(path)
    write_video_frames(output, frames(), fps=float(fps), codec="libx264")
    return output
...
def render_suite_contact_sheet(task_summaries: Sequence[str | Path], path: str | Path) -> Path:
    """Render exactly four task summary PNGs; task cards retain best/worst labels."""
    ...
    for index, (axis, summary) in enumerate(zip(axes.flat, task_summaries), start=1):
        image = plt.imread(Path(summary))
        axis.imshow(image); axis.set_axis_off()
        stem = Path(summary).stem.lower()
        label = "WORST" if "worst" in stem else "BEST" if "best" in stem else "BEST/WORST audit"
        axis.set_title(f"Task {index} • {label}", loc="left", fontsize=12, fontweight="bold")
