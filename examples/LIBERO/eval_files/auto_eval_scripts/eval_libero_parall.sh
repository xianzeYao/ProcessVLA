...
export LIBERO_python=${LIBERO_PYTHON:-python3}  # Path to the Python environment
export starVLA_python=${STARVLA_PYTHON:-${STAR_VLA_PY:-python3}}  # Path to the Python environment

# === End of environment variable configuration ===
export LIBERO_CONFIG_PATH=${LIBERO_HOME}/libero  # Path to LIBERO configuration files
export PYTHONPATH=$PYTHONPATH:${LIBERO_HOME} # let eval_libero find the LIBERO tools
export PYTHONPATH=$(pwd):${PYTHONPATH} # let LIBERO find the websocket tools from ProcessVLA repo
###########################################################################################
...
CUDA_VISIBLE_DEVICES=$gpu_id "${starVLA_python}" "${PROCESSVLA_ROOT}/deployment/model_server/server_policy.py" \
    --ckpt_path "${your_ckpt}" \
    --port "${base_port}" \
    --use_bf16 &
...
"${LIBERO_python}" "${PROCESSVLA_ROOT}/examples/LIBERO/eval_files/eval_libero.py" \
