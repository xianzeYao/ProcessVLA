...
run_root_dir=${RUN_ROOT_DIR:-${PROCESSVLA_ROOT}/results/cotrain_runs}
wandb_entity=${WANDB_ENTITY:-your_wandb_entity}
wandb_project=${WANDB_PROJECT:-starVLA4Libero}
pretrained_checkpoint=${PRETRAINED_CHECKPOINT:-}
resume_from_checkpoint=${RESUME_FROM_CHECKPOINT:-}
run_id=${RUN_ID:-libero4in1_qwen2.5gr00t_cotrain}
config_file=${CONFIG_FILE:-${PROCESSVLA_ROOT}/starVLA/config/deepseeds/deepspeed_zero2.yaml}
train_script=${TRAIN_SCRIPT:-${PROCESSVLA_ROOT}/starVLA/training/train_starvla_cotrain.py}
...
if [[ -n "${resume_from_checkpoint}" ]]; then
  ...
  extra_args+=(--trainer.resume_from_checkpoint: "${resume_from_checkpoint}")
fi
